"""Sensor entities for the Airco Bridge."""
...
from .const import DIAGNOSTIC_SENSORS, MAIN_SENSORS, is_valid_temperature
...
async def async_setup_entry(
    ...
    entities: list[SensorEntity] = [AircoBridgeStatusSensor(coordinator, entry)]
...
class AircoBridgeStatusSensor(CoordinatorEntity, SensorEntity):
...
class AircoBridgeFieldSensor(CoordinatorEntity, SensorEntity):
    ...
    def available(self) -> bool:
        ...
        if self._attr_device_class == "temperature":
            return is_valid_temperature(_read_path(self.coordinator.data, self._path))
    ...
    def native_value(self):
        ...
        if self._attr_device_class == "temperature":
            return value if is_valid_temperature(value) else None
