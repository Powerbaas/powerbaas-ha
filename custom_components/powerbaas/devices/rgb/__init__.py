"""Powerbaas RGB device support for the Powerbaas integration."""
...
from .client import RgbClient
from .const import CONF_DEVICE_URL, DEFAULT_POLL_INTERVAL

_LOGGER = logging.getLogger(__name__)


class RgbCoordinator(DataUpdateCoordinator):
    ...
    def __init__(
        ...
        self._offline_issue_id = f"rgb_offline_{entry.entry_id}"

    async def _async_update_data(self) -> dict[str, Any]:
        status = await self.client.async_get_status()
        if status is None:
            self._register_failure()
            raise UpdateFailed(f"Powerbaas RGB status request failed for {self.device_name}")

        self._register_success()
        system = await self.client.async_get_system()
        return {
            "rgb": status.get("rgb") or {},
    ...
    def _register_failure(self) -> None:
        ...
        if self._consecutive_failures == OFFLINE_AFTER_CONSECUTIVE_FAILURES:
            ...
            _LOGGER.warning(
                "Powerbaas RGB offline for %s after %s consecutive failed fetches",
                self.device_name,
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> dict:
    """Set up an Powerbaas RGB device and return its runtime data."""
    device_url = entry.data.get(CONF_DEVICE_URL)
    if not device_url:
        raise ConfigEntryNotReady("No device URL configured for Powerbaas RGB.")

    device_name = entry.title or "Powerbaas RGB"
    client = RgbClient(hass, device_url)
    ...
    coordinator = RgbCoordinator(hass, entry, client, device_name)
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> None:
    ...
    issue_registry.async_delete_issue(hass, DOMAIN, f"rgb_offline_{entry.entry_id}")
